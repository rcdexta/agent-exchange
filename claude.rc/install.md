...
Set `AX_VERSION=v0.5.1` in the installer's environment to select a specific release. Set `AX_INSTALL_DIR` to select another binary directory, or `AX_NO_MODIFY_PATH=1` to manage PATH yourself. For these options, download the script to a file and invoke it with the environment variables set.
